"""
Evaluate Condition Sensitivity Gap (CSG) for edge-conditioned generation.

CSG = EAS_IoU(matched conditions) - EAS_IoU(mismatched conditions)
"""
...
def _as_binary_edge_mask(x: torch.Tensor, threshold: float) -> torch.Tensor:
...
def _cyclic_mismatch(condition_batch: torch.Tensor) -> torch.Tensor:
    if condition_batch.shape[0] <= 1:
        return condition_batch
    idx = (torch.arange(condition_batch.shape[0], device=condition_batch.device) + 1) % condition_batch.shape[0]
    return condition_batch[idx]


def _batch_iou(
    samples: torch.Tensor,
    target_condition: torch.Tensor,
    eval_edge_method: str,
    ...
    generated_edges = _extract_edges_from_generated(
        samples=samples,
        edge_method=eval_edge_method,
        canny_sigma=canny_sigma,
        canny_low=canny_low,
        canny_high=canny_high,
        xdog_sigma=xdog_sigma,
    )
    pred = _as_binary_edge_mask(generated_edges, threshold=mask_threshold)
    target = _as_binary_edge_mask(target_condition, threshold=mask_threshold)

    pred_d = _dilate_binary(pred, tolerance_px)
    target_d = _dilate_binary(target, tolerance_px)
    inter_iou = (pred_d & target_d).flatten(1).sum(dim=1).float()
    union = (pred_d | target_d).flatten(1).sum(dim=1).float()
    return inter_iou / (union + 1e-8)


def main():
    parser = argparse.ArgumentParser(description="Evaluate Condition Sensitivity Gap (CSG).")
    ...
    matched_iou_scores: List[float] = []
    mismatched_iou_scores: List[float] = []

    with torch.no_grad():
        pbar = tqdm(range(0, args.num_samples, args.batch_size), desc="CSG eval")
        for start in pbar:
            end = min(start + args.batch_size, args.num_samples)
            condition_batch = condition_all[start:end]
            mismatch_batch = _cyclic_mismatch(condition_batch)
            batch_size = condition_batch.shape[0]

            # Reset RNG so matched/mismatched generations share noise trajectory.
            torch.manual_seed(args.seed + start)
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all(args.seed + start)
            samples_matched = method.sample(
            ...
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all(args.seed + start)
            samples_mismatched = method.sample(
                batch_size=batch_size,
                image_shape=image_shape,
                num_steps=num_steps,
                condition=mismatch_batch,
                method=args.sampler,
                eta=args.eta,
            )

            iou_matched = _batch_iou(
                samples=samples_matched,
                target_condition=condition_batch,
            ...
            iou_mismatched = _batch_iou(
                samples=samples_mismatched,
                target_condition=condition_batch,
                eval_edge_method=args.eval_edge_method,
                mask_threshold=args.mask_threshold,
                tolerance_px=args.tolerance_px,
            ...
            matched_iou_scores.extend(iou_matched.cpu().tolist())
            mismatched_iou_scores.extend(iou_mismatched.cpu().tolist())

    def _mean_std(xs: List[float]) -> Tuple[float, float]:
        t = torch.tensor(xs, dtype=torch.float32)
        return float(t.mean().item()), float(t.std(unbiased=False).item())

    matched_mean, matched_std = _mean_std(matched_iou_scores)
    mismatched_mean, mismatched_std = _mean_std(mismatched_iou_scores)
    csg = matched_mean - mismatched_mean
    ...
    result: Dict[str, object] = {
        ...
        "mask_threshold": args.mask_threshold,
        "tolerance_px": args.tolerance_px,
        "eas_iou_matched_mean": matched_mean,
        "eas_iou_matched_std": matched_std,
        "eas_iou_mismatched_mean": mismatched_mean,
        "eas_iou_mismatched_std": mismatched_std,
        "condition_sensitivity_gap": csg,
    }
    ...
    if args.save_log_path:
        ...
        print(f"Saved CSG results to {save_path}")
