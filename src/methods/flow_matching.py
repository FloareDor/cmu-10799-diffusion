...
class FlowMatching(BaseMethod):
    ...
    def compute_loss(self, x_1: torch.Tensor, condition: Optional[dict] = None) -> Tuple[torch.Tensor, Dict[str, float]]:
        """
        Compute the flow matching loss with optional conditioning.
        
        Args:
            x_1: Clean data samples (batch_size, channels, height, width)
            condition: Dictionary containing conditioning info (e.g., {'audio': ..., 'image': ...})
        ...
        # 5. Model prediction
        # Scale t from [0, 1] to [0, num_timesteps] 
        t_input = t * (self.num_timesteps - 1)
        
        # Pass x_t, t, and condition to the model
        if condition is not None:
            v_theta = self.model(x_t, t_input, **condition)
        else:
    ...
    def sample(
        ...
        **kwargs
        ...
        """
        Generate samples using Euler integration, optionally conditioned.
        
        Args:
            batch_size: Number of samples to generate
            image_shape: (channels, height, width)
            num_steps: Number of Euler steps
            condition: Dictionary containing conditioning info
