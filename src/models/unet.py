...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        audio_cond_dim: int = 0, # Dimension of audio embedding (0 = no audio)
    ):
        super().__init__()
        ...
        self.audio_cond_dim = audio_cond_dim
        ...
        # Audio Projection
        if audio_cond_dim > 0:
            self.audio_proj = nn.Linear(audio_cond_dim, time_embed_dim)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, image: Optional[torch.Tensor] = None, audio: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Forward pass of the UNet.
        
        Args:
            x: Input tensor usually (B, 3, H, W)
            t: Timestep tensor
            image: Condition image (masked face), (B, 3, H, W). If provided, concatenated to x.
            audio: Audio embedding (B, audio_cond_dim). If provided, added to time embedding.
        """
        ...
        # Add audio embedding
        if audio is not None and hasattr(self, 'audio_proj'):
            t_emb = t_emb + self.audio_proj(audio)
